"""Convert generic raw handoff data to the state26 -> absjoint18 LeRobot contract."""
...
def _write_portable_fallback(raw_dir: Path, output_dir: Path, episodes: list[Path], fps: float, split: dict[str, list[str]]) -> None:
    ...
    (meta_dir / "tasks.jsonl").write_text(json.dumps({"task_index": 0, "task": LANGUAGE_INSTRUCTION}) + "\n", encoding="utf-8")
...
def convert(raw_dir: str | Path, output_dir: str | Path | None = None, *, train_split: float = 0.9, repo_id: str | None = None, allow_portable_fallback: bool = True) -> dict[str, Any]:
    ...
    output_dir = Path(output_dir) if output_dir else LEROBOT_ROOT / f"{dataset_name}_state26_absjoint18"
    ...
    repo_id = repo_id or f"local/{dataset_name}_state26_absjoint18"
    ...
    dataset_manifest = {
        "contract": LEROBOT_CONTRACT_VERSION,
        "writer": writer,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Convert raw handoff data to state26 -> absjoint18 LeRobot dataset.")
