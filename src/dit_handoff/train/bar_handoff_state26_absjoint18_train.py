"""Training wrapper for rectangular-bar handoff state26 -> absjoint18."""
...
def build_training_config(dataset_dir: Path, profile: dict[str, Any], *, run_name: str, smoke: bool) -> dict[str, Any]:
    ...
    if manifest.get("contract") != BAR_HANDOFF_LEROBOT_CONTRACT_VERSION:
        raise ValueError(f"dataset contract must be {BAR_HANDOFF_LEROBOT_CONTRACT_VERSION}")
    ...
    return {
        ...
        "dataset_tolerance_s": float(profile.get("dataset_tolerance_s", 1e-4)),
        "run_name": run_name,
...
def build_lerobot_train_command(
    ...
    *,
    ...
    command = [
        ...
        f"--tolerance_s={config['dataset_tolerance_s']}",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run LeRobot MultiTask DiT training for bar_handoff_state26_absjoint18.")
    parser.add_argument("dataset_dir", type=Path)
    parser.add_argument("--profile", type=Path, default=None)
    parser.add_argument("--run-name", default="bar_handoff_state26_absjoint18_official_time")
