"""Closed-loop evaluation for rectangular-bar handoff state26 -> absjoint18."""
...
from dit_handoff.collect.raw_collector import _policy_obs, _to_serializable
from dit_handoff.collect.raw_collector_bar_handoff import _observation_snapshot, _warmup_cameras
from dit_handoff.constants import ACTION_DIM, BAR_HANDOFF_JOINT_POS_TASK_ID, BAR_HANDOFF_LANGUAGE_INSTRUCTION, CAMERA_OBS_FEATURES, EVAL_ROOT
    ...
    def __init__(self, checkpoint: Path, policy_python: str, device: str, out_dir: Path):
        ...
        self.proc = subprocess.Popen(
            [
                policy_python,
                "-m",
                "dit_handoff.eval.policy_worker_bar_handoff_state26_absjoint18",
                "--checkpoint",
                str(checkpoint),
                "--device",
                device,
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        obs = _warmup_cameras(env, obs, args.camera_warmup_steps, env.unwrapped.device)
        ...
        for step_index in range(args.max_steps):
            ...
            if not action_queue:
                if args.smoke_hold_current:
                    left = snapshot["arms"]["left"]["joint_pos"]
                    right = snapshot["arms"]["right"]["joint_pos"]
                    chunk = torch.tensor([[left + right]], dtype=torch.float32)
                else:
                    chunk = _policy_action_chunk(policy_stack, snapshot, obs, BAR_HANDOFF_LANGUAGE_INSTRUCTION, args.device)
                chunk_list = chunk.tolist()[0]
                execute_steps = len(chunk_list)
                if override is not None:
                    execute_steps = min(execute_steps, int(override))
                action_queue = [list(map(float, act)) for act in chunk_list[:execute_steps]]
                call_row = {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Closed-loop eval for bar_handoff state26 -> absjoint18.")
    try:
        from isaaclab.app import AppLauncher
    ...
    parser.add_argument("--task", default=BAR_HANDOFF_JOINT_POS_TASK_ID)
